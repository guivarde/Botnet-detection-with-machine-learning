'''
Code pour le bucketing des netflows dans les fenêtres temporelles.
'''
...
def bucket(filename, timeWindow = 60):
	if os.path.exists('/home/guive/Documents/Dataset/Current_ICSX_dataset/ndf/'+filename[64:-4]+'_ndf.csv'):
	...
	#rendre l'heure de début et l'heure de fin absolues
	ref=min(df.unixstarttime)
	df.unixstarttime=df.unixstarttime-ref
	df.unixstoptime=df.unixstoptime-ref

	#trier les données en fonction de  ['unixstarttime' 'unixstoptime']
	...
	ndf.to_csv('/home/Guive/Documents/Dataset/Current_training_ICSX_dataset/ndf/'+filename[64:-4]+'_ndf.csv')
	print filename[64:-4]+'_ndf.csv done!'

filename=glob.glob("/home/guive/Documents/Dataset/Current_training_ICSX_dataset/flow/*")
#print filename
for f in filename:
	bucket(f)
